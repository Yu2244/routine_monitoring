...
function GraphScreen({ history, checklist, graphData }) {
  ...
  const graphableQuestions = useMemo(() => {
    return checklist?.questions?.filter(q =>
      isGraphableType(q.type) && q.graphConfig?.enabled // Check type and enabled flag
    ) || [];
  }, [checklist]);
        ...
        {graphableQuestions.map((q, index) => {
          const data = graphData ? graphData[q.id] : [];
           if (!data || data.length === 0) {
               const graphTitle = q.graphConfig?.customName ? `${q.graphConfig.customName} (${q.text})` : q.text;
               return (
           ...
           const graphTitle = `${q.graphConfig.customName} (${q.text})`;
              ...
              <ResponsiveContainer width="100%" height={300}>
                {q.type === 'yes_no' && (
                  <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    ...
                    {/* Dynamically set legend name based on window size */}

...
export function useWiki() {
  ...
  const createPage = useCallback(async () => {
    ...
    const result = await showInputPrompt({
        title: '新規ページ作成',
        message: '新しいページ名を入力してください (英数字、ハイフンのみ):',
        defaultValue: 'new-page'
    });

    if (result.success && result.value) {
      const newPageName = result.value.trim();
      // More robust validation for filename (allow only safe characters)
      if (!/^[a-zA-Z0-9-]+$/.test(newPageName)) {
           alert("無効なページ名です。英数字とハイフンのみ使用できます。"); return;
       }
